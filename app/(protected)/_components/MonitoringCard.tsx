...
export default function MonitoringCard({
  ...
  return (
    <div
      className="space-y-4 rounded-2xl border p-6  bg-background
                    transition-all duration-300 ease-out
                    hover:shadow-md hover:-translate-y- "
    >
      <h1 className="text-center font-bold text-3xl">{label}</h1>

      <div className="grid grid-cols-2 gap-4">
        <div className={`rounded-xl border p-4 ${className1}`}>
          <p className="text-sm text-muted-foreground">Total Data Entri</p>
        ...
        <div className={`rounded-xl border p-4 ${className2}`}>
          <p className="text-sm text-muted-foreground">Total Data Masuk</p>
