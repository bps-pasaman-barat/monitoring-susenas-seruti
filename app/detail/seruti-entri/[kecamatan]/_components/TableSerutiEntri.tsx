...
export default function TableSerutiEntri({ kecamatan }: { kecamatan: string }) {
  ...
  const [data, setData] = useState<SerutiEntriResponse | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `/api/backend/seruti-entri/${kecamatan}?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}`,
        );

        if (!res.ok) throw new Error("Fetch gagal");
        const result: SerutiEntriResponse = await res.json();
        setData(result);
      } catch (err) {
        console.error(err);
      } finally {
