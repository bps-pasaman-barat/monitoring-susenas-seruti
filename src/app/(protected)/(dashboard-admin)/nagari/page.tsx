...
const fetcher = async (url: string) => {
  ...
  if (!res.ok) {
    // biome-ignore lint/suspicious/noExplicitAny: custom error object
...
export default function NagariPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      mutate(
        (key) =>
          typeof key === "string" && key.startsWith("/api/default-nagari"),
      );
    } catch (err: any) {
      ...
      <div className="p-6 bg-white rounded-md border space-y-4">
            ...
            <DialogContent>
              ...
              <form onSubmit={handleCreate} className="space-y-4 mt-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={newKecamatanId}
                    onValueChange={setNewKecamatanId}
                  >
                    <SelectTrigger>
        ...
        <div className="flex justify-between items-center mt-4">
          <div className="text-sm text-muted-foreground">
            Halaman {metadata.page} dari {metadata.totalPages} ({metadata.total}{" "}
            data)
          </div>
          <div className="flex gap-2">
            <Button
