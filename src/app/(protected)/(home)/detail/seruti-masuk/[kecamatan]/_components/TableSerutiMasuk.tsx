...
import type { SerutiMasukResponse } from "@/types";

export default function TableSerutiMasuk({ kecamatan }: { kecamatan: string }) {
  ...
  const [data, setData] = useState<SerutiMasukResponse | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `/api/backend/seruti-masuk/${kecamatan}?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}`,
        );
        if (!res.ok) throw new Error("Fetch gagal");
        const result: SerutiMasukResponse = await res.json();
        setData(result);
      } catch (err) {
        console.error(err);
      } finally {
