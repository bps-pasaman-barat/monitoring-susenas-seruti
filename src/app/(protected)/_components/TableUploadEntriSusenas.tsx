...
import type { UploadSusenasEntriResponse } from "@/types";

export default function TableUploadEntriSusenas() {
  ...
  async function handleDelete(filename: string) {
    try {
      const res = await fetch(
        `/api/backend/uploaded-susenas/entri/${filename}`,
        { method: "DELETE" },
      );
      ...
      mutate(
        (key) =>
          typeof key === "string" &&
          key.startsWith("/api/backend/uploaded-susenas/entri"),
      );
    } catch (_err) {
  ...
  const key = `/api/backend/uploaded-susenas/entri?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}`;
