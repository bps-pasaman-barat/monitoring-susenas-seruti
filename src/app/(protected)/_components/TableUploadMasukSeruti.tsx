...
export default function TableUploadMasukSeruti() {
  ...
  async function handleDelete(filename: string) {
    try {
      const res = await fetch(
        `/api/backend/uploaded-seruti/masuk/${filename}`,
        { method: "DELETE" },
      );
      ...
      mutate(
        (key) =>
          typeof key === "string" &&
          key.startsWith("/api/backend/uploaded-seruti/masuk"),
      );
    } catch (_err) {
  ...
  const key = `/api/backend/uploaded-seruti/masuk?page=${page}&limit=${limit}&orderBy=${orderBy}&order=${order}`;
